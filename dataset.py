...
def get_bts_data_lists(data_dir: str):
    """
    ...
    # Define the absolute base path validated on Kaggle
    base_val = "/kaggle/input/datasets/awsaf49/brats20-dataset-training-validation/BraTS2020_ValidationData/MICCAI_BraTS2020_Validation_Data/"

    # Retrieve all patient directories matching the expected pattern
    patient_dirs_val = sorted(glob.glob(os.path.join(base, "BraTS20_Validation_*")))
...
def get_dataloaders(data_dir:str,batch_size:int=1,roi_size=(64,64,64)):
    ...
    dataset_val = Dataset(data_list, transform=transforms)
